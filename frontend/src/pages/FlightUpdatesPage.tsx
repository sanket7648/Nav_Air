...
//   actualTime: string | null; // ISO 8601 datetime string or null
//   status: string;
//   gate: string | null;
...
//   // Effect to fetch flight data on component mount and every 60 seconds thereafter
//   useEffect(() => {
//     fetchFlights();
//     const intervalId = setInterval(fetchFlights, 60000); // Refresh every 60 seconds
...
//                 <p className="mt-1 text-sm text-gray-500">There are no flights matching your current filter.</p>
//             </div>
//           ) : (
...
//               return (
//                 <div key={flight.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-lg transition-shadow duration-300">
//                   <div className="flex items-start justify-between mb-4">
...
//                       </div>
//                     </div>
//                     <button onClick={() => toggleNotification(flight.id)} className={`p-2 rounded-full transition-colors ${ isNotificationEnabled ? 'bg-yellow-100 text-yellow-600' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'}`}>
...
//                             <div className="text-base font-semibold text-gray-900">{formatTime(flight.scheduledTime)}</div>
//                         </div>
//                         <div>
//                             <div className="text-xs text-gray-500 mb-1">Estimated</div>
//                             <div className={`text-base font-semibold ${flight.delay && flight.delay > 0 ? 'text-red-500' : 'text-gray-900'}`}>{formatTime(flight.actualTime)}</div>
//                         </div>
//                          <div>
...
//                       {getTimeUntilFlight(flight.scheduledTime)}
//                   </div>
//                 </div>
//               );
//             })
                ...
                <div key={flight.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-lg transition-shadow duration-300">
                  ...
                  <div className="border-t border-gray-100 pt-4">
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        <div>
                            <div className="text-xs text-gray-500 mb-1">Scheduled</div>
                            <div className="text-base font-semibold text-gray-900">{formatTime(flight.scheduledDeparture)}</div>
                        </div>
                        <div>
                            <div className="text-xs text-gray-500 mb-1">Estimated</div>
                            <div className={`text-base font-semibold ${flight.delay && flight.delay > 0 ? 'text-red-500' : 'text-gray-900'}`}>{formatTime(flight.actualDeparture)}</div>
                        </div>
                         <div>
                            <div className="text-xs text-gray-500 mb-1">Gate</div>
                            <div className="text-base font-semibold text-gray-900 flex items-center">
